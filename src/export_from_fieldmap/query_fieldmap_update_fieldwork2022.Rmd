...
query_relocation <- "SELECT REPLACINGPOINTS.ID,
 ...
 replacements_orig <- sqlQuery(connect_db, query_relocation)
...
sample_status_old <- read_vc(file = "sample_status", root = "../../processed/fieldmap_mhq") %>%
  left_join(select(date_assessment_old, plot_id, date_assessment), by = "plot_id") %>%
...
sample_status_old_select <- sample_status_old %>%
  group_by(plot_id) %>%
  filter(date_status == max(date_status)) %>%
  ungroup() %>%
  select(-navplot_id, -veg_done, -reg_done, -dendro_done, -remark4, -springveg, -info_springveg) %>%
  filter(!(plot_id == "1296105" & is.na(remark)))

check_double <- sample_status_old_select %>%
  group_by((plot_id)) %>%
  filter(n() > 1)

# bij bos nog verouderde categorieën --> aanpassen
sample_status_old_forest <- sample_status_old_select %>%
  filter(db == "foresthab") %>%
...
sample_status_old <- sample_status_old_select %>%
  filter(db == "openhab") %>%
  bind_rows(sample_status_old_forest)
...
type_observed_old <- read_vc(file = "type_observed", root = "../../processed/fieldmap_mhq")  %>%
  inner_join(select(sample_status_old_select, plot_id, date_assessment), by = "plot_id") %>%
  select(plot_id, date_assessment, everything())
...
type_observed_update <- type_observed_update %>%
...
cover_species_old <-  read_vc(file = "cover_species", root = "../../processed/fieldmap_mhq")  %>%
  inner_join(select(sample_status_old, plot_id, date_assessment), by = "plot_id") %>%
  select(plot_id, date_assessment, everything())
...
cover_veglayers_old <-  read_vc(file = "cover_veglayers", root = "../../processed/fieldmap_mhq")  %>%
  inner_join(select(sample_status_old, plot_id, date_assessment, status_fieldwork, info_status_fieldwork), by = "plot_id") %>%
  select(plot_id, date_assessment, everything())
...
#v2022 
assessments_openhab_v2022 <- read_vc("assessments_openhab", "../../processed/fieldmap_mhq/fieldwork2022")

sample_status <- read_vc("sample_status", "../../processed/fieldmap_mhq") 
type_observed <- read_vc("type_observed", "../../processed/fieldmap_mhq")
cover_species <- read_vc("cover_species", "../../processed/fieldmap_mhq")
structure_vars <- read_vc("structure_vars", "../../processed/fieldmap_mhq")
trees_a3a4 <- read_vc("trees_a3a4", "../../processed/fieldmap_mhq")
trees_a2 <- read_vc("trees_a2", "../../processed/fieldmap_mhq")

sample_status_old <- sample_status %>%
  filter(year(date_assessment) < 2022)

type_observed_old <- type_observed %>%
  filter(year(date_assessment) < 2022)

cover_species_old <- cover_species %>%
  filter(year(date_assessment) < 2022)

structure_vars_old <- structure_vars %>%
  filter(year(date_assessment) < 2022)

type_observed_old_plot <- type_observed_old %>%
  mutate(type_observed_circle_description = str_c(ifelse(is.na(type_observed_cover_circle), "?", type_observed_cover_circle), "% ", type_observed_circle)) %>%
...
assessments_old <- sample_status_old %>%
    left_join(type_observed_old_plot, by = c("plot_id", "date_assessment")) %>%
  #opname uitgevoerd cfr invav
    mutate( inaccessible = ifelse(!is.na(info_status_fieldwork) 
            ...
            not_measurable = ifelse(!is.na(info_status_fieldwork) 
                                   & info_status_fieldwork %in% c("opname tijdelijk onmogelijk (gemaaid/te nat/ondoordringbaar)"), "short term", 
                                  ifelse(info_status_fieldwork %in% c("opname onmogelijk (gemaaid/begraasd/te nat/ondoordringbaar)"), "long term", NA)),
            assessment_source = ifelse(is.na(inaccessible) & is.na(not_measurable), "field assessment", NA),
            lsvi_measurement = info_status_fieldwork == "100% afgewerkt",
            completed = ifelse(lsvi_measurement,
                              ifelse(!is.na(info_status_fieldwork) &  info_status_fieldwork != "100% afgewerkt", FALSE, TRUE),NA), 
         ) %>%
  mutate(plot_id = as.character(plot_id),
         cover_species_record = plot_id %in% cover_species_old$plot_id,
         structure_openhab_record = ifelse(db == "openhab", plot_id %in% structure_vars_old$plot_id, NA),
         dendro_record = ifelse(db == "foresthab", plot_id %in% c(trees_a2$plot_id, trees_a3a4$plot_id), NA)) %>%
  filter(!(lsvi_measurement & is.na(type_observed)))

assessments_old <- assessments_old %>%
  mutate(assessment_source = ifelse(!is.na(not_measurable) & not_measurable == "Short term" & cover_species_record, "field assessment", assessment_source),
         status_fieldwork = ifelse(status_fieldwork == "Geen opname uitgevoerd" & cover_species_record, "Opname gedeeltelijk uitgevoerd", status_fieldwork)) %>%
  rename(assessment_date = date_assessment)


path <- fileman_up("n2khab-sample-admin")

mhq_assessments_orig <- read_vc(file = "mhq_terr_assessments", root = file.path(path, "data"))

mhq_assessments <- mhq_assessments_orig %>%
  filter(!is.na(assessment_date)) %>%
  filter(fieldwork_team == "anb") %>%
  filter(assessment_source != "orthophoto") %>%
  mutate(refpoint_n = as.numeric(str_sub(point_code, start = -1)),
         plot_id = str_remove(point_code, "_2"),
         plot_id = str_remove(plot_id, "_3"),
         plot_id = str_remove(plot_id, "_4"),
         plot_id = str_remove(plot_id, "_1"))

mhq_assessments <- mhq_assessments %>%
  group_by(plot_id) %>%
  filter(refpoint_n == max(refpoint_n)) %>%
  ungroup()

assessments_old_diff <- mhq_assessments %>%
  anti_join(assessments_old, by = c("plot_id"))

mhq_assessments_diff <- assessments_old %>%
  anti_join(mhq_assessments, by = c("plot_id"))
...
```{r}
check <- assessments_old %>%
  filter(lsvi_measurement, db == "foresthab") %>%
  group_by(type_observed) %>%
  summarise(n = n()) %>%
  ungroup()

check_forest_lsvi <- assessments_old %>%
  filter(db == "foresthab") %>%
  filter(lsvi_measurement) %>%
  select(plot_id, lsvi_measurement, type_observed, everything())

check_forest_assess <- assessments_old %>%
  filter(db == "foresthab") %>%
  select(plot_id, lsvi_measurement, type_observed, everything())



measurements_forest <- read_vc(file = "mhq_terr_measurements", root = file.path(path, "data")) %>%
  filter(str_detect(type_observed, "91")) %>%
  mutate(plot_id = str_remove(point_code, "_2"),
         plot_id = str_remove(plot_id, "_1"))

assessments_forest <- read_vc(file = "mhq_terr_assessments", root = file.path(path, "data")) %>%
  filter(str_detect(type_target, "91")) %>%
  mutate(plot_id = str_remove(point_code, "_2"),
         plot_id = str_remove(plot_id, "_1")) %>%
  filter(!is.na(assessment_date))
  
missing_1 <- measurements_forest %>%
  anti_join(check_forest_lsvi, by = "plot_id")

missing_2 <- assessments_forest %>%
  anti_join(check_forest_assess, by = "plot_id")

missing_3 <- check_forest_assess %>%
  anti_join(assessments_forest)

check <- measurements_forest %>%
  group_by(plot_id) %>%
  filter(n() > 1)
```
