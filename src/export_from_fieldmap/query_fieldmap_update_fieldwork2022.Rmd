...
sample_status <- sample_status %>%
  select(-habt1, -habsubt1, -x_m, - y_m) %>%
  filter(remark != "opname door INBO ikv verdroging" | is.na(remark))

assessments_openhab <- check_missing_data %>%
  filter(db == "openhab") %>%
  filter(remark != "opname door INBO ikv verdroging" | is.na(remark)) %>%
    select(plot_id, sampling_unit_code, x_m, y_m, targethabitat, status_fieldwork,info_status_fieldwork, assessment_source, date_assessment, inaccessible, not_measurable, type_observed,  lsvi_measurement, remark, structure_openhab_record, cover_species_record)
...
Does this mean that an othet type is observed than the target type nad that it still has to be determined if the plot is a valid sampling unit for the observed type?
...
measured_plot_heide <- assessments_openhab %>%
  ...
  select(plot_id, x = x_m, y = y_m, date_assessment, type_observed)
