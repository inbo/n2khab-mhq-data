...
sample_status_foresthab_past2020 <- sample_status_foresthab_past2020_orig %>%
  ...
  select(mon_cycle, plot_id, date_status, info_status_fieldwork, veg_done, reg_done, dendro_done, info_springveg, springveg, remark4, remark, db) %>%
...
assessments <- check_missing_data %>%
  mutate(date_assessment = if_else(is.na(date_assessment), as.Date(date_status), date_assessment)) %>%
    select(mon_cycle, plot_id, sampling_unit_code, targethabitat, status_fieldwork,info_status_fieldwork, assessment_source, date_assessment, inaccessible, not_measurable, type_observed,  lsvi_measurement, remark, structure_openhab_record, cover_species_record, dendro_record) %>%
