...
db_fieldmap_foresthab <- file.path(fileman_up("n2khab-mhq-data"), "raw/fieldmap_foresthab/fieldmap_foresthab_v2023.FDB")
...
## Functions to access forest hab fieldmap database - VBI3

```{r}
get_plot_status_vbi3 <- function(db = db_fieldmap_foresthab){

  query_invcycle <- "
    SELECT invcycle.IDPlots as plot_id,
    qplotstatus.value1 as plotstatus,
    qaccessibility.value1 as accessibility,
    qconlanduse.value1 as landuse,
    qconlandcategory.value1 as landcategory,
    qconforestareatype.value1 as forestareatype
    FROM invcycle
    ...
    LEFT JOIN qconlanduse ON invcycle.landuse = qconlanduse.id
    LEFT JOIN qconlandcategory ON invcycle.landcategory = qconlandcategory.id
    LEFT JOIN qconforestareatype ON invcycle.forestareatype = qconforestareatype.id;
  "
  connect_db <- odbcDriverConnect(paste("DRIVER={Firebird/InterBase(r) driver};UID=SYSDBA;PWD=masterkey; DBNAME=", db))
...
get_treesa3a4_vbi3 <- function(db = db_fieldmap_foresthab){
  ...
  treesa3a4 <- sqlQuery(connect_db, query_trees_vbi3, stringsAsFactors = FALSE)
...
plot_status_vbi3 <- get_plot_status_vbi3()
trees_vbi3 <- get_treesa3a4_vbi3()
```
...
colnames(sample_status_openhab); colnames(sample_status_foresthab)
  ...
  #opname uitgevoerd cfr invav
    mutate( assessment_source = ifelse(targethabitat %in% c("ja", "nee") | (!is.na(type_observed)), 
            ...
            lsvi_measurement = ifelse(assessment_source == "field assessment" & 
                                         !(info_status_fieldwork  %in% c("te hoge RANKING", "voldoende opnames beschikbaar", "opname nog niet 100% klaar")),
                                ifelse(!is.na(targethabitat) & targethabitat == "ja", TRUE, FALSE),
                                FALSE), 
            lsvi_measurement = ifelse(inaccessible %in% c("short term", "long term") | not_measurable %in% c("short term", "long term"), FALSE, lsvi_measurement), 
...
assessments_openhab <- check_missing_data %>%
  ...
  mutate(date_assessment = if_else(is.na(date_assessment), as.Date(date_status), date_assessment)) %>%
    select(plot_id, sampling_unit_code, targethabitat, status_fieldwork,info_status_fieldwork, assessment_source, date_assessment, inaccessible, not_measurable, type_observed,  lsvi_measurement, remark, structure_openhab_record, cover_species_record)
...
write_vc(type_observed_corr, "type_observed", root = path,  sorting = c("plot_id", "segment_id", "type_observed_circle", "type_observed_square"), strict = FALSE)
...
write_vc(coordinates, "coordinates", root = path,  sorting = "plot_id")
write_vc(trees_a3a4, "trees_a3a4", root = path,  sorting = c("plot_id", "segment_id"))
write_vc(trees_a2, "trees_a2", root = path,  sorting = c("plot_id", "name_nl"), strict = FALSE)
write_vc(logs, "logs", root = path,  sorting = c("plot_id", "idline_intersect_method"))
write_vc(check_missing_data, root = path,  "data_overview", sorting = "plot_id")
