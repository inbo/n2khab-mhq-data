...
db_fieldmap_openhab_2 <- file.path(fileman_up("n2khab-mhq-data"), "raw/fieldmap_openhab/fieldmap_openhab_v8_cycle2.FDB")
db_fieldmap_openhab_2024 <- file.path(fileman_up("n2khab-mhq-data"), "raw/fieldmap_openhab/access_openhab_v2024.accdb")
...
if (!file.exists(db_fieldmap_openhab_2024)) {
...
query_gridpoints_accdb <- "SELECT Grid_points.IDPlots as navplot_id,
...
query_gridpoints_fb <- "SELECT Grid_points.IDPlots as navplot_id,
    Grid_points.Ranking as plot_id,
    Grid_points.Add_date as date_status,
    Grid_points.SingleID as sampling_unit_code,
    Grid_points.Habsubt1,
  ...
  if (str_sub(db, nchar(db) - 3, nchar(db)) == ".mdb") {
    ...
    gridpoints_orig <- sqlQuery(connect_db, query_gridpoints_accdb)
    ...
    gridpoints_orig <- sqlQuery(connect_db, query_gridpoints_fb)
 ...
 gridpoints <- gridpoints_orig %>%
    ...
    mutate(type_target_all = ifelse(!is.na(habt2), str_c(habsubt1, habt2, sep = "; "),
                                    ifelse(!is.na(habsubt2) & habsubt2 != "0", str_c(habsubt1, habsubt2, sep = "; "),
                                           habsubt1))) %>%
   select(-habsubt2, -habsubt1, -habt2)
...
SiteDescription_HEIDE.Edit_date as date_sitedescription,
  ...
  LEFT JOIN Standdescription S ON SiteDescription_HEIDE.IDSTANDDESCRIPTION = S.ID AND SiteDescription_HEIDE.IDplots = S.IDPlots;"
...
# uitgezonderd bedekking van struik+boomlaag worden alle bedekkingen via BMS-schaal bepaald
  structure_var_bms <- structure_vars %>%
    ...
    gather(-plot_id, - segment_id, -date_sitedescription, -area_m2, key = "structure_var", value = "class_id") %>%
    mutate(coverscale_name = "Beheermonitoringsschaal") %>%
      left_join(bms2, by = c("class_id")) %>%
      select(plot_id, segment_id, area_m2, structure_var, coverscale_name, cover_description, cover_mean) %>%
      mutate(structure_var = tolower(structure_var))
  
  structure_var_cover <- structure_vars %>%
      mutate(structure_var = "shrub_treelayer") %>%
      select(plot_id, segment_id, area_m2, structure_var, cover = shrub_and_treelayer_18m)
...
get_coordinates_openhab <- function(db = db_fieldmap_openhab){
  ...
  GPS_REF.Remark as remark_coord
  FROM GPS_REF"
...
get_coordinates_openhab_c2 <- function(db = db_fieldmap_openhab_2){
  ...
  GPS_REF.Y_m as y_measured,
  GPS_REF.verlegd,
  GPS_REF.NR_VERVANGPLOT,
  GPS_REF.Remark as remark_coord,
  rp.RANKING as ranking_replacement
  FROM GPS_REF
  LEFT OUTER JOIN Replacingpoints rp ON (RP.Point_code = gps_ref.uniekeid) AND (RP.rank_rel_b = gps_ref.nr_vervangplot);"
  ...
  coordinates <- coordinates %>%
    unique() %>%
    mutate(plot_id = ifelse(str_detect(point_code, "_"),
                            str_sub(point_code, end = str_locate(point_code, "_")[,"start"] - 1),
                            point_code),
           plot_id = as.numeric(plot_id),
           point_code = as.character(point_code))
...
gridpoints <- gridpoints_orig %>%
    ...
    mutate(status_fieldwork = ifelse(status_fieldwork == 2, "nee", 
           ...
           type_target_all = ifelse(!is.na(extra_subtype), str_c(hab_subtype, "; ", extra_subtype),
                                    hab_subtype)) %>%
  select(-hab_subtype, -extra_subtype)
...
get_coordinates_foresthab <- function(db = db_fieldmap_foresthab){
  ...
  Plots_Forest_Inventory.remark as remark_coord
...
get_treesa3a4_raw <- function(db = db_fieldmap_foresthab){
  ...
  Trees_2eBosinv.Species as tree_species_id,
  qTreeSpecies.Value1 as name_nl,
  qTreeSpeciesScientific.Value1 as name_sc,
  qStatusTree.Value1 as status_tree,
  qCoppice_Individual.Value1 as coppice_individual,
  qIntactTree.Value1 as intact_tree
  FROM Trees_2eBosinv 
  LEFT JOIN qTreeSpeciesScientific ON Trees_2eBosinv.Species_Scientific = qTreeSpeciesScientific.ID
...
get_shoots <- function(db =  db_fieldmap_foresthab){

query_shoots <- "
  SELECT Shoots_2eBosinv.IDPlots as plot_id
  , Shoots_2eBosinv.IDTrees_2eBosinv as tree_id
  , Shoots_2eBosinv.ID as shoot_id
  , Shoots_2eBosinv.Perimeter_cm
  , Shoots_2eBosinv.Height_m as height_shoot_m
  , Shoots_2eBosinv.DBH_MM
  FROM Shoots_2eBosinv;
"
connect_db <- odbcDriverConnect(paste("DRIVER={Firebird/InterBase(r) driver};UID=SYSDBA;PWD=masterkey; DBNAME=", db))

shoots <- sqlQuery(connect_db, query_shoots, stringsAsFactors = TRUE);

odbcClose(connect_db)

colnames(shoots) <- str_to_lower(colnames(shoots))

return(shoots)  

}

get_standdescription <- function(db = db_fieldmap_foresthab){
  
  query_stand <- "SELECT 
                  Standdescription_segments.IDPlots as plot_id,
                  Standdescription_segments.ID as segment_id,
                  Standdescription_segments.Area_m2 as area_a4_m2,
                  qLanduse.Value1 as landuse,
                  qStandType.Value1 as stand_type,
                  qStandAge.Value1 as stand_age,
                  qMixType.Value1 as mix_type
                  FROM Standdescription_segments
                  INNER JOIN qStandType ON qStandType.ID = Standdescription_segments.StandType
                  INNER JOIN qLanduse ON qLanduse.ID = Standdescription_segments.Landuse
                  INNER JOIN qStandAge ON qStandAge.ID = Standdescription_segments.StandAge
                  INNER JOIN qMixType ON qMixType.ID = Standdescription_segments.MixType;"
  
  connect_db <- odbcDriverConnect(paste("DRIVER={Firebird/InterBase(r) driver};UID=SYSDBA;PWD=masterkey; DBNAME=", db))

  standdescription <- sqlQuery(connect_db, query_stand, stringsAsFactors = FALSE) %>%
    unique()

  odbcClose(connect_db)
  
  colnames(standdescription) <- str_to_lower(colnames(standdescription))
  ...
  colnames(plot_status) <- str_to_lower(colnames(plot_status))
   ...
   plot_status <- plot_status %>%
    mutate(habitat_target = gsub(" ", "_", habitat_target)) %>%
     rename(type_target_all = habitat_target)
...
get_treesa3a4_past2020 <- function(db = db_fieldmap_foresthab_past2020){
  ...
  Trees_3.VOL_STEM_M3,
  Trees_3.VOL_TOT_M3,
  Trees_3.species as tree_species_id,
  qTreeSpecies.Value1 as name_nl,
  qTreeSpeciesScientific.Value1 as name_sc,
  qStatusTree.Value1 as status_tree,
  qCoppice_Individual.Value1 as coppice_individual,
  ...
  LEFT JOIN qTreeSpeciesScientific ON Trees_3.Species_scientific = qTreeSpeciesScientific.ID
...
get_standdescription_past2020 <- function(db = db_fieldmap_foresthab_past2020){
  
  query_stand <- "SELECT 
                  Standdescription_segments_3.IDPlots as plot_id,
                  Standdescription_segments_3.ID as segment_id,
                  Standdescription_segments_3.Area_m2 as area_a4_m2,
                  qStandType.Value1 as stand_type,
                  qStandAge.Value1 as stand_age,
                  qStanddescr_landuse.value1 as landuse,
                  qMixType.Value1 as mix_type
                  FROM Standdescription_segments_3
                  INNER JOIN qStandType ON qStandType.ID = Standdescription_segments_3.StandType
                  INNER JOIN qStandAge ON qStandAge.ID = Standdescription_segments_3.StandAge
                  INNER JOIN qStanddescr_landuse ON qStanddescr_landuse.ID = Standdescription_segments_3.landuse
                  INNER JOIN qMixType ON qMixType.ID = Standdescription_segments_3.MixType;"
  ...
  standdescription <- standdescription %>%
    mutate(landuse = str_to_lower(landuse))
...
sample_status_openhab <- get_status_openhab() %>%
    mutate(db = "openhab_c1",
           mon_cycle = 1)

sample_status_openhab_c2 <- get_status_openhab(db = db_fieldmap_openhab_2) %>%
    mutate(db = "openhab_c2",
           mon_cycle = 2) 

sample_status_openhab_2024 <- get_status_openhab(db = db_fieldmap_openhab_2024) %>%
    mutate(db = "openhab_c2",
           mon_cycle = 2) 

check_doubles <- sample_status_openhab_c2 %>%
  semi_join(sample_status_openhab, by = c("plot_id", "date_status"))
...
sample_status_foresthab_past2020 <- sample_status_foresthab_past2020_orig %>%
  ...
  mutate(remark = str_trim(remark),
         info_status_fieldwork = ifelse(dendro_record & cover_species_record, "100% afgewerkt",
                                        ifelse(dendro_record | cover_species_record, "opname nog niet 100% klaar",
                                               ifelse(type_observed_circle != type_target_all & !is.na(type_observed_circle) & type_observed_circle != "onbekend", "Geen opname : geen doelhabitat", 
  ...
  select(mon_cycle, plot_id, date_status, info_status_fieldwork, type_target_all, veg_done, reg_done, dendro_done, info_springveg, springveg, remark4, remark, db) %>%
  filter(!is.na(info_status_fieldwork))

sample_status <- bind_rows(sample_status_openhab, 
                           sample_status_openhab_c2,
                           sample_status_openhab_2024,
...
type_observed <- get_type_observed_openhab() %>%
  mutate(mon_cycle = 1) %>%
    bind_rows(get_type_observed_openhab(db = db_fieldmap_openhab_2) %>%
                mutate(mon_cycle = 2)) %>%
  bind_rows(get_type_observed_openhab(db = db_fieldmap_openhab_2024) %>%
                mutate(mon_cycle = 2)) %>%
...
check <-  type_observed %>%
    group_by(mon_cycle, plot_id, type_observed_circle, type_observed_square, landuse) %>%
    mutate(n = n()) %>%
    ungroup() %>%
    filter(n > 1)
...
check_na <- type_observed_corr %>%
  group_by(mon_cycle, plot_id) %>%
  filter(any(is.na(type_observed_cover_circle)))
...
cover_veglayers <- get_cover_veglayers_openhab() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_cover_veglayers_openhab(db_fieldmap_openhab_2) %>%
  mutate(mon_cycle = 2)) %>%
  bind_rows(get_cover_veglayers_openhab(db_fieldmap_openhab_2024) %>%
  mutate(mon_cycle = 2)) %>%
...
cover_veglayers <- cover_veglayers %>%
  filter(!(segment_id > 1 & is.na(cover))) %>%
  select(-segment_id) %>%
  unique()

#check only one record per layer
check <- cover_veglayers %>%
  group_by(mon_cycle, plot_id, layer) %>%
  filter(n() > 1)
...
cover_species <- get_cover_species_openhab() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_cover_species_openhab(db_fieldmap_openhab_2) %>%
  mutate(mon_cycle = 2)) %>%
    bind_rows(get_cover_species_openhab(db_fieldmap_openhab_2024) %>%
  mutate(mon_cycle = 2)) %>%
...
structure_vars_heath <- get_measurements_heath_circle() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_measurements_heath_circle(db = db_fieldmap_openhab_2) %>%
              mutate(mon_cycle = 2)) %>%
  bind_rows(get_measurements_heath_circle(db = db_fieldmap_openhab_2024) %>%
              mutate(mon_cycle = 2))

structure_vars_6510 <- get_measurements_6510_circle() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_measurements_6510_circle(db = db_fieldmap_openhab_2) %>%
              mutate(mon_cycle = 2)) %>%
  bind_rows(get_measurements_6510_circle(db = db_fieldmap_openhab_2024) %>%
              mutate(mon_cycle = 2))

structure_vars <- bind_rows(structure_vars_heath, structure_vars_6510)

check <- structure_vars_heath %>%
    group_by(mon_cycle, plot_id, segment_id, structure_var) %>%
    mutate(n = n()) %>%
    ungroup() %>%
    filter(n > 1)
...
standdescription <- get_standdescription() %>%
  bind_rows(get_standdescription_past2020())
...
date_assessment <- get_date_openhab() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_date_openhab(db = db_fieldmap_openhab_2) %>%
              mutate(mon_cycle = 2)) %>%
  bind_rows(get_date_openhab(db = db_fieldmap_openhab_2024) %>%
              mutate(mon_cycle = 2)) %>%
  bind_rows(date_assessment_forest) %>%
  unique() %>%
  group_by(mon_cycle, plot_id, date_vegetation, date_dendro) %>%
...
coordinates  <- get_coordinates_openhab() %>%
  mutate(mon_cycle = 1) %>%
  bind_rows(get_coordinates_openhab_c2(db = db_fieldmap_openhab_2) %>%
              mutate(mon_cycle = 2)) %>%
  bind_rows(get_coordinates_openhab_c2(db = db_fieldmap_openhab_2024) %>%
              mutate(mon_cycle = 2)) %>%
  ...
  bind_rows(get_coordinates_foresthab(db = db_fieldmap_foresthab_past2020) %>%
              mutate(mon_cycle = 1)) %>%
  unique() %>%
  mutate(verlegd = verlegd == 1) %>%
  select(mon_cycle, plot_id, point_code, everything()) %>%
  select(-idplots) %>%
  filter(!is.na(plot_id))
...
The type in the centre of the plot determines if the sampling unit should be measured or not.

The type is recorded in the square plot and the circle plot.

We assume that the type in the centre of the plot corresponds with the type recorded in the square plot. If the type in the square plot is missing, we assume that the type corresponds with the type recorded in the circle plot. If the circle plot is a mixed plot we assume that the segment with the lowest number contains the centre of the plot.
...
type_observed_plot <- type_observed_corr %>%
  ...
  mutate(type_observed = ifelse(!is.na(type_observed_square), type_observed_square, type_observed_circle),
         type_observed = ifelse(type_observed_square %in% main_types$type & 
                                  type_observed_circle %in% subtypes$type &
                                  str_sub(type_observed_circle, 1, 4) == str_sub(type_observed_square, 1, 4),
                                type_observed_circle, type_observed)) %>%
...
```{r}
check_type %>%
    select(mon_cycle, plot_id, type_observed, type_observed_circle_description) %>%
    datatable(filter = "top",
            rownames = FALSE)
```
...
check_missing_data <- sample_status_most_recent %>%
    filter(!is.na(plot_id)) %>%
    left_join(type_observed_plot, by = c("plot_id", "mon_cycle")) %>%
    left_join(date_assessment_plot, by = c("plot_id", "mon_cycle")) %>%
  #opname uitgevoerd cfr invav
    mutate( inaccessible = ifelse(!is.na(info_status_fieldwork) 
         ...
         structure_openhab_record = ifelse(str_detect(db, "openhab") | is.na(db), plot_id %in% structure_vars$plot_id, NA)) %>%
...
assessments <- check_missing_data %>%
  mutate(date_assessment = if_else(is.na(date_assessment), as.Date(date_status), date_assessment)) %>%
    select(mon_cycle, plot_id, sampling_unit_code, type_target_all, targethabitat, status_fieldwork,info_status_fieldwork, assessment_source, date_assessment, inaccessible, not_measurable, type_observed,  lsvi_measurement, remark, structure_openhab_record, cover_species_record, dendro_record) %>%
...
check_missing_data_openhab <- check_missing_data %>%
    filter(db %in% c("openhab_c1", "openhab_c2")) %>%
    ...
    select(plot_id, mon_cycle, date_assessment, status_fieldwork, info_status_fieldwork, remark, type_observed_circle,  coordinates_record, cover_species_record, structure_openhab_record) 
...
check_missing_data_openhab2 <- check_missing_data %>%
    filter(db %in% c("openhab_c1", "openhab_c2") | is.na(db)) %>%
  #  filter(!lsvi_measurement | status_fieldwork == "nee" | is.na(status_fieldwork)) %>%
    ...
    filter(cover_species_record |
               structure_openhab_record) %>%
    select(plot_id, date_assessment, mon_cycle,  status_fieldwork, info_status_fieldwork, remark, type_observed_circle, , cover_species_record, structure_openhab_record) 
...
check_missing_data_foresthab <- check_missing_data %>%
    filter(db == c("foresthab", "foresthab_2020")) %>%
    filter(lsvi_measurement) %>%
    filter(is.na(type_observed_circle) |
               is.na(date_assessment) |
               !cover_species_record |
               !dendro_record |
               !a2_record) %>%
    select(plot_id, date_assessment, mon_cycle, status_fieldwork, info_status_fieldwork, remark, type_observed_circle, cover_species_record, dendro_record, a2_record, a3a4_record) 
...
check_missing_data_foresthab2 <- check_missing_data %>%
    filter(db %in% c("foresthab", "foresthab_past2020")) %>%
...
check_missing_data_foresthab3 <- check_missing_data %>%
    filter(db %in% c("foresthab", "foresthab_past2020") | is.na(db)) %>%
...
sample_status_date <- sample_status_most_recent %>%
  left_join(select(date_assessment, plot_id, date_assessment, mon_cycle), by = c("plot_id", "mon_cycle")) %>%
...
type_observed_date <- type_observed_corr %>%
  unique() %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = c("plot_id", "mon_cycle")) %>%
  select(plot_id, segment_id, mon_cycle, date_assessment, everything())

check <- type_observed_date %>%
  group_by(plot_id, segment_id, mon_cycle) %>%
  filter(n() > 1)

cover_species_date <- cover_species %>%
  mutate(name_sc = ifelse(name_nl == "Tormentil", "Potentilla erecta (L.) Räuschel", name_sc),
         name_sc = ifelse(name_nl == "Valse voszegge", "Carex cuprina (Sándor ex Heuffel) Nendtvich ex A. Kerner", name_sc),
         name_sc = ifelse(name_nl == "Gewone reigersbek s.s.", "Erodium cicutarium (L.) L'Hérit. subsp. cicutarium", name_sc)) %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = c("plot_id", "mon_cycle")) %>%
  select(plot_id, mon_cycle, date_assessment, everything())

check <- cover_species_date %>%
  group_by(plot_id, mon_cycle, layer, name_nl) %>%
  filter(n() > 1)
...
check <- cover_veglayers_date %>%
  group_by(plot_id, layer, mon_cycle) %>%
  filter(n() > 1)
...
check <- structure_vars_date %>%
  group_by(plot_id, segment_id, structure_var, mon_cycle) %>%
  filter(n() > 1)
...
logs_date <- logs %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = c("plot_id")) %>%
  select(plot_id, mon_cycle, date_assessment, everything()) 

trees_a3a4_date <- trees_a3a4 %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = c("plot_id")) %>%
  select(plot_id, segment_id, tree_id, mon_cycle, date_assessment, everything()) 
...
trees_a2_date <- trees_a2 %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = "plot_id") %>%
  select(plot_id, mon_cycle, date_assessment, everything())
...
check_missing_data <- check_missing_data %>%
  select(-navplot_id) %>%
  select(plot_id, segment_id, mon_cycle, date_status, everything())

standdescription_date <- standdescription %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = "plot_id") %>%
  select(plot_id, mon_cycle, date_assessment, everything()) %>%
  mutate(landuse =ifelse(plot_id == 11126 & segment_id == 2, "andere (stort, grondwinning)", landuse)) %>%
  pivot_longer(cols = c("stand_age", "stand_type", "mix_type"), names_to = "variable", values_to = "category")
...
write_vc(cover_veglayers_date, "cover_veglayers", root = path,  sorting = c("plot_id", "date_assessment", "layer"), strict = FALSE)
...
write_vc(date_assessment, "date_assessment", root = path,  sorting = c("plot_id", "mon_cycle"), strict = FALSE)
write_vc(coordinates_date, "coordinates", root = path,  sorting = c("plot_id", "date_assessment"), strict = FALSE)
write_vc(trees_a3a4_date, "trees_a3a4", root = path,  sorting = c("plot_id", "segment_id", "tree_id", "date_assessment"), strict = FALSE)
write_vc(trees_a2_date, "trees_a2", root = path,  sorting = c("plot_id", "date_assessment", "name_nl"), strict = FALSE)
write_vc(standdescription_date, "standdescription", root = path,  sorting = c("plot_id", "segment_id", "date_assessment", "variable"), strict = FALSE)
write_vc(shoots_date, "shoots", root = path,  sorting = c("plot_id", "tree_id", "shoot_id"), strict = FALSE)
...
write_vc(assessments, root = path,  "assessments", sorting = c("plot_id", "date_assessment", "mon_cycle"), strict = FALSE)
write_vc(check_missing_data_foresthab, root = path,  "check_missing_data_foresthab", sorting = c("plot_id", "mon_cycle"), strict = FALSE)
write_vc(check_missing_data_openhab, root = path,  "check_missing_data_openhab", sorting = c("plot_id", "mon_cycle"), strict = FALSE)
