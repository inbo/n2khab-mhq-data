...
standdiscription_date <- standdescription %>%
  inner_join(select(sample_status_date, plot_id, date_assessment, mon_cycle), by = "plot_id") %>%
  select(plot_id, mon_cycle, date_assessment, everything()) %>%
  mutate(landuse =ifelse(plot_id == 11126, "andere (stort, grondwinning)", landuse))
...
write_vc(standdiscription_date, "standdiscription_date", root = path,  sorting = c("plot_id", "segment_id", "date_assessment"), strict = FALSE)
