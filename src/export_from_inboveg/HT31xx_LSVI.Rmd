...
# invoervereisten LSVI-rekenmodule stilstaande wateren (versie 3)
...
site_characteristics_join <- site_characteristics %>% 
  ...
  # controle: AN_KAL_007_3160_16 VZuSS 1 Vzuur 10 --> eind Vzuur = 1 (is ok)
...
data_soortenkenmerken <- vegetation %>% 
  ...
  mutate(name = case_when(
    name == "Chara hispida L." ~ "Chara hispida",
    # Chara hispida L. wordt niet herkend door berekenLSVIbasis; er wordt aangeraden auteursnaam te verwijderen
    TRUE ~ name)) %>%
  semi_join(data_habitat, by = c("recording_givid" = "ID")) %>% 
