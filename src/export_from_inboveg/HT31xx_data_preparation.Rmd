...
```{r}
no_classification <- anti_join(header_31xx, classif_31xx, by = "recording_givid") # to do: 1 opname: op te volgen (2190)
...
structure_class <- structure_31xx %>% 
  ...
  # to do: 31xx_lsvi_vertikalestructuur; code 25 --> wat is dit? voorlopig geschrapt
...
```{r}
# to do: moet eigenlijk opgelost worden in INBOVEG
tbl_var <- site_characteristics %>% 
  select(var_code, var2 = var) %>% 
  filter(!is.na(var2)) %>% 
  distinct()

tbl_value_description <- site_characteristics %>% 
  select(value, vd2 = value_description) %>% 
  filter(!is.na(vd2)) %>% 
  distinct()

site_characteristics <- site_characteristics %>%  
  left_join(tbl_var, by = "var_code") %>% 
  mutate(var = var2) %>% 
  select(-var2) %>% 
  left_join(tbl_value_description, by = "value") %>% 
