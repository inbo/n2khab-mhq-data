...
```{r}
stilstaande_typen <- c("2190", "3110", "3130_aom", "3130_na", "3140", "3150", "3160")
# to do: subtype 2190a toevoegen als dit beschikbaar is in inboveg
...
tmp - to do later weglaten
```{r}
sort(unique(structure_31xx$var_code))
sort(unique(structure_31xx$var_value_code))
# to do: opvolgen of toevoeging "permw" bij class en "subst" bij multiple goed verwerkt wordt
```
...
layerqual <- layerqual_31xx %>% 
  ...
  mutate(is_below_LOQ = FALSE, 
         ...
         is_numeric = TRUE,
         unit = "%")
# to do: check of toevoegingen van lagen van 2190a correct worden weergegeven:
# SlSH, SlSNH, Boom, BooOv, Eutro, MonoR
```
...
tmp - to do later weglaten
```{r}
sort(unique(layerqual_31xx$var_code))
```
