...
db_analyse <- "vbi_analysedatabank_v2023-08-22.accdb"
...
cover_veglayers_vbi <- cover_species_vbi %>%
  ...
  summarise(cover_mean = 1 - prod(1 - cover_mean/100)) %>%
  ungroup() %>%
...
trees_a2 <- boomsoort_orig %>%
  select(plot_id = IDPlots, segment_id = IDSegments, vbi_cycle = Periode, stem_density_a2_ha = v6_StamtalA2_ha) %>%
...
write_vc(trees_a3a4, "trees_a3a4", root = path, sorting = c("plot_id", "vbi_cycle", "segment_id", "tree_id"))
...
write_vc(plot_weight_complete, "plot_weight", root = path, sorting = c("plot_id", "segment_id", "vbi_cycle"), strict = FALSE)
