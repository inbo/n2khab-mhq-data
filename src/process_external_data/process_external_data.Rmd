...
check_coordinates <- coordinaten_orig %>%
  filter(periode > 1) %>%
  group_by(idplots) %>%
  filter(n_distinct(periode) >= 2) %>%
  ungroup() %>%
  filter(!is.na(idgroup))

check_coordinates_paired <- check_coordinates %>%
  select(idplots, idgroup, periode, type, x, y) %>%
  pivot_wider(names_from = periode, values_from = c(x, y, type)) %>%
  mutate(distance = sqrt((x_2 - x_3) ^ 2 + (y_2 - y_3) ^ 2))
