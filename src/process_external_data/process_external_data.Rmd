...
# msa 91E0_vc geldt ook voor 91E0_va -> toevoegen
vbi_msa_91E0_va <- vbi_msa %>%
  filter(type == "91E0_vc") %>%
  mutate(type = "91E0_va")

vbi_msa <- vbi_msa %>%
  bind_rows(vbi_msa_91E0_va) %>%
  arrange(plot_id, periode, type)
...
# msa 91E0_vc geldt ook voor 91E0_va -> toevoegen
mhq_msa_91E0_va <- mhq_msa %>%
  filter(type == "91E0_vc") %>%
  mutate(type = "91E0_va")

mhq_msa <- mhq_msa %>%
  bind_rows(mhq_msa_91E0_va) %>%
  arrange(plot_id, date_assessment, type)
